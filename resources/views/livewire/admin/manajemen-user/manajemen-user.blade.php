{{-- filepath: resources/views/livewire/admin/manajemen-user/manajemen-user.blade.php --}}
...
>
    ...
    <h1 class="text-3xl font-bold mb-8 text-white tracking-wide animate-fade-in-down flex items-center gap-4">
        <img src="{{ asset('img/sidebar/profil.webp') }}" alt="Admin" class="w-10 h-10" />
        Manajemen User
    </h1>
                ...
                <div x-data="{ open: false }" class="relative group w-full md:w-48 ">
                    ...
                    <div x-show="open" x-transition class="absolute z-30 mt-2 w-full bg-white rounded-lg shadow-lg border border-gray-200">
                        <ul>
                            ...
                            <li>
                                <button wire:click="$set('filterStatus', 'aktif')" @click="open = false"
                                    class="cursor-pointer flex items-center w-full px-4 py-2 hover:bg-green-50 rounded-lg transition text-green-700 font-semibold">
                                    <span class="mr-2">✅</span> Aktif
                                </button>
                            </li>
                            <li>
                                <button wire:click="$set('filterStatus', 'nonaktif')" @click="open = false"
                ...
                <tbody>
                    <template x-for="(user, i) in paginatedUsers" :key="user.id">
                        <tr class="hover:bg-blue-50 transition border-b border-blue-100">
                            ...
                            <td class="py-3 px-6" x-text="user.name"></td>
                            ...
                            <td class="py-3 px-6">
                                ...
                                <template x-if="['Admin','HR'].includes(user.role?.name)">
                                    <span class="text-gray-800 text-base pl-14">-</span>
                                </template>
                            </td>
                            <td class="py-3 px-6"
                                x-text="(() => {
                            ...
                            <td class="py-3 px-6 justify-center gap-2">
                                <template x-if="!['Admin','HR'].includes(user.role?.name)">
                                    <div class="inline">
                                        <button
                    ...
                    <template x-if="paginatedUsers.length === 0">
                        <tr>
                            <td colspan="8" class="py-6 text-center text-gray-800">Tidak ada user ditemukan.</td>
                        </tr>
                    </template>
                </tbody>
            </table>
    ...
    >
        <div class="bg-linear-to-r from-[#F53003] to-[#0074D9] rounded-xl p-8 shadow-lg w-full max-w-md relative">
            <button @click="showDeleteModal = false" class="absolute top-2 right-2 text-gray-400 hover:text-red-500 text-xl cursor-pointer">&times;</button>
            ...
            <div class="flex justify-end gap-2">
                <button @click="showDeleteModal = false" class="px-4 py-2 rounded bg-linear-to-r from-[#F53003] to-[#0074D9]   text-gray-700 font-semibold cursor-pointer hover:scale-105 transition-all">Batal</button>
                <button
