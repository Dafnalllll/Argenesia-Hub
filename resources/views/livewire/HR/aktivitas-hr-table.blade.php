            ...
            <tbody>
                @forelse ($aktivitasHR as $aktivitas)
                    <tr class="hover:bg-pink-100 transition text-center animate-fade-in-up">
                        ...
                        <td class="py-1 px-2 md:py-2 md:px-4 whitespace-nowrap">
                            @php
                                $keterangan = $aktivitas->keterangan;
                                $keterangan = str_replace(
                                    ['Ditolak', 'Disetujui', 'Menunggu'],
                                    [
                                        '<span class="bg-red-100 text-red-700 border border-red-400 rounded px-1">Ditolak</span>',
                                        '<span class="bg-green-100 text-green-700 border border-green-400 rounded px-1">Disetujui</span>',
                                        '<span class="bg-yellow-100 text-yellow-700 border border-yellow-400 rounded px-1">Menunggu</span>',
                                    ],
                                    $keterangan
                                );
                            @endphp
